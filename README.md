...
Проект использует npm workspaces. Приложение в `apps/web` названо `@checkout/web`. Корневой `npm run dev` запускает только API; фронтенд — в отдельном терминале, см. раздел ниже.
...
Данные сохраняются в `.data/store.json`. Запускайте один экземпляр API на один файл. Для сброса остановите сервер и выполните `npm run data:reset` (команда учитывает `DATA_FILE`, если он задан); затем создайте новую сессию.
